"""ikt_corrector.py — IKT as a TORSION corrector.
...
Why torsions and not coordinates (measured, 2026-07-12/13):
  ...
  * ADT's own parametrisation already exposes exactly this degree of freedom (see CONVENTION below):
    the ADD vector that placed each atom is edited in DIRECTION only, never in length, so a pure torsion
    rotates the whole subtree rigidly about the bond and cannot change bond lengths OR bond angles. One
    scalar per atom, native to the token stream. (The optional bend head moves the one angle it must.)
...
CONVENTION (torsion + bend = modifying the ADD vector's direction, length fixed):
  ADT places atom k with an ADD vector (parent -> k) written in the PARENT's local frame as
  (r, h0, h1, h2) = (bond length, direction). In that frame the x-axis is (grandparent -> parent) and the
  direction is just spherical coordinates on it:
      bend    = the angle the ADD vector makes WITH the x-axis   (polar;  = angle grandparent-parent-k)
      torsion = its rotation ABOUT the x-axis                    (azimuthal)
  The IKT edits ONLY this direction and NEVER r: the bond length is invariant by construction
  (forward_kinematics rotates the vector and a rotation preserves its norm). A pure torsion also leaves
  every bond angle unchanged -- it rotates the whole subtree rigidly about the bond -- while the bend head
  additionally nudges the single angle grandparent-parent-k. So every output stays a valid conformer of
  the same topology; only dihedral (and small bond-angle) freedoms move. In short: it just rotates the
  vector each atom was ADDed with, keeping its length.
  Verified end-to-end: measuring torsions()/bendangles() on conformer i and applying the measured
  (dtheta, dbend) through forward_kinematics reconstructs conformer j to ~0.002-0.03 A RMSD (roundtrip).
"""
...
class TorsionIKT(nn.Module):
    ...
    def forward(self, tokens_list, atom_idx_list, device, want_bend=False):
        ...
        vals, slots, acts, pad = build_inputs(tokens_list, device)
