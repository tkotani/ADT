...
def expand_cluster(start_atom, mol, visited):
    """Find all atoms connected to start_atom via non-single bonds (transitive).
    Returns list with start_atom first, then others in BFS order,
    plus parent_map for intra-cluster connections."""
    from rdkit import Chem
    cluster = [start_atom]
    cluster_parent = {start_atom: None}  # None = parent comes from outside
    queue = [start_atom]
    seen = {start_atom}
    while queue:
        atom = queue.pop(0)
        for bond in mol.GetAtomWithIdx(atom).GetBonds():
            other = bond.GetOtherAtomIdx(atom)
            if other in seen or other in visited:
                continue
            if bond.GetBondType() != Chem.BondType.SINGLE:
                seen.add(other)
                cluster.append(other)
                cluster_parent[other] = atom
                queue.append(other)
    return cluster, cluster_parent
...
def freeorder_decompose(mol, chain, atomR):
    ...
    # v21: expand clusters for bootstrap atoms
    for bs_atom in list(chain) + [atomR]:
        cluster, cluster_parents = expand_cluster(bs_atom, mol, visited)
        for c_atom in cluster[1:]:  # skip bs_atom itself (already visited)
            visited.add(c_atom)
            atom_order.append(c_atom)
            if cluster_parents[c_atom] is None:
                parent_map[c_atom] = bs_atom
            else:
                parent_map[c_atom] = cluster_parents[c_atom]
    ...
    while frontier:
        ...
        for atom, parent in frontier:
            if atom in visited:
                continue
            # v21: expand cluster (non-single-bond connected atoms)
            cluster, cluster_parents = expand_cluster(atom, mol, visited)
            for c_atom in cluster:
                visited.add(c_atom)
                atom_order.append(c_atom)
                if cluster_parents[c_atom] is None:
                    parent_map[c_atom] = parent  # external parent from frontier
                else:
                    parent_map[c_atom] = cluster_parents[c_atom]  # intra-cluster
            placed_any = True
            # Add single-bond neighbors of ALL cluster atoms to frontier
            for c_atom in cluster:
                for nb in adj[c_atom]:
                    if nb not in visited:
                        new_frontier.append((nb, c_atom))
            break
