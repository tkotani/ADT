...
_DATAGEOM = _os_v.path.abspath(_os_v.path.join(_HERE, "../data/geom"))   # dataset loader (load_drugs) lives with the data
for _p in (_DATAGEOM, _COMMON, _HERE):      # common first, HERE last -> HERE at sys.path[0] => wins
    if _p in _sys_v.path:
        _sys_v.path.remove(_p)
    _sys_v.path.insert(0, _p)
from adt_model import build_model, ADTv2Model, N_ACTIONS          # version-local emb_offset model
...
import relative_pointer                                            # common: absolute->offset conversion
from tokenizer import tokenize_molecule                     # copied into this dir
from load_drugs import load_drugs_mols                             # ../data/geom (dataset loader+builder)
...
_OFFSET_OVERFLOW_N = 0
_ADD_ACTIONS = {ADD_INIT, ADD_CHAIN, ADD_ANGLE, ADD}


def _max_parent_offset(arr, n_slots=N_SLOTS):
    """Max parent OFFSET in a relative-encoded token array (slot1 for ADD-family, slot2 for LINK).
    offset > max_offset => this free-order references a parent too far back to encode."""
    m = 0
    for s in range(len(arr) // n_slots):
        a = int(arr[s * n_slots])
        if a in _ADD_ACTIONS:
            m = max(m, int(arr[s * n_slots + 1]))
        elif a == LINK:
            m = max(m, int(arr[s * n_slots + 2]))
    return m


def _warn_offset_overflow(smi, na, max_offset):
    global _OFFSET_OVERFLOW_N
    _OFFSET_OVERFLOW_N += 1
    if _OFFSET_OVERFLOW_N <= 10 or _OFFSET_OVERFLOW_N % 500 == 0:
        import warnings
        warnings.warn(f"[free-order] all 15 retries needed offset > max_offset={max_offset} "
                      f"(n_atoms={na}, {smi}) -> skipping molecule [{_OFFSET_OVERFLOW_N} skipped so far]")
...
class FreeOrderDrugsDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Free-order = a RANDOM atom order; offsets depend on the order. Retry several orders so
        # a parent that lands > max_offset back in one order can land closer in another. If EVERY
        # retry still overflows max_offset, warn + SKIP the molecule (return [END]) -- otherwise the
        # model would silently CLAMP the offset to a WRONG parent. For max30 data offset<=29<max_offset
        # (50) so this never triggers; it guards the unbounded-atom (long-molecule) regime.
        # (The v21 tokenize_molecule signature is (mol, positions, root, seed) -- no max_offset kwarg;
        # enforcement lives here, in the consumer, against the offset of the relative encoding.)
        overflowed = False
        for attempt in range(15):
            try:
                ...
                if result is not None and result.chain_length == 1:
                    ...
                    arr = relative_pointer.absolute_to_relative(arr)  # v2 emb_offset: store parent OFFSET at slot1/LINK-slot2
                    # step 3 == LINK => the triple is itself a 3-ring; skip & retry.
                    if len(arr) > 3 * N_SLOTS and int(arr[3 * N_SLOTS]) == LINK:
                        continue
                    if self.max_offset is not None and _max_parent_offset(arr) > self.max_offset:
                        overflowed = True
                        continue   # parent > max_offset back in this order -> try a different free-order
                    return torch.tensor(arr, dtype=torch.long)
            except Exception:
                pass

        if overflowed:
            _warn_offset_overflow(self.smiles[idx], n, self.max_offset)
        return torch.tensor([END], dtype=torch.long)
...
def generate_one(model, device, frame_sampler=None, max_steps=200, temperature=1.0):
    ...
    # v2 emb_offset: the model re-embeds slot1 / slot2-LINK as OFFSETS, so the token stream fed back to it
    # must carry offsets. frame_arr above (absolute) was already consumed for geometry reconstruction;
    # convert ONLY the model-input copy here. atom_table / bonds / from_id / to_id stay ABSOLUTE for geometry.
    # (absolute_to_relative gives the SAME frame offsets the trainer's __getitem__ produces -> consistent.)
    use_in_offset = getattr(model, 'input_pointer_mode', 'absolute') == 'offset'
    if use_in_offset:
        tokens = [int(x) for x in relative_pointer.absolute_to_relative(np.asarray(tokens, dtype=np.int64))]
    ...
    with torch.no_grad():
        for step in range(max_steps):
            ...
            if getattr(model, 'output_pointer_mode', 'absolute') == 'offset':
                ...
                if max_c < 0:
                    from_id = 1
                    tokens.append((n_atoms + 1 - from_id) if use_in_offset else from_id)
                else:
                    ...
                    from_id = n_atoms - c
                    # store OFFSET (c+1) for emb_offset input; from_id stays absolute for geometry below
                    tokens.append((c + 1) if use_in_offset else from_id)
            else:
            ...
            for slot in range(2, 7):
                ...
                # LINK slot-2 (offset OUTPUT): sampled class c_to -> absolute to_id = n_atoms - c_to.
                # For emb_offset INPUT the token stream stores the OFFSET (c_to+1); the absolute to_id is
                # recovered for the bond below. (Legacy absolute-input model stores the absolute to_id.)
                if slot == 2 and action == LINK and \
                        getattr(model, 'output_pointer_mode', 'absolute') == 'offset':
                    c_to = val
                    val = (c_to + 1) if use_in_offset else (n_atoms - c_to)
                tokens.append(val)
            ...
            if action == ADD:
                ...
                # tokens[step_base+2] holds the OFFSET under emb_offset input -> recover absolute to_id
                # (= n_atoms - offset + 1, the relative_pointer LINK inverse); legacy stores absolute.
                _v = tokens[step_base + 2]
                to_id = (n_atoms - (_v - 1)) if use_in_offset else _v
...
def main():
    ...
    parser.add_argument('--max_offset', type=int, default=32,
                        help='[v26 primary] Output pointer softmax size (number of '
                             ...
                             'The OUTPUT-side capacity limit, bounded '
    ...
    train_ds = FreeOrderDrugsDataset(molecules, split='train', n_val=500, max_offset=args.max_offset)
    val_ds = FreeOrderDrugsDataset(molecules, split='val', n_val=500, max_offset=args.max_offset)
    ...
    # Model (same as DFS). emb_offset arch: model sizing is max_offset (INPUT emb_offset + OUTPUT
    # head width). This model has no emb_pointer.
    config = {
        'd_model': args.d_model, 'n_heads': args.n_heads,
        'n_layers': args.n_layers, 'd_ff': args.d_ff,
        'dropout': args.dropout, 'nohydrogen': args.nohydrogen, 'n_r_bins': 200,
        'output_pointer_mode': 'offset',
        'input_pointer_mode': 'offset',           # v2: emb_offset input (this dir's model is offset-only)
        'max_offset': args.max_offset,
        'arch_version': 'vtakao202606231610',     # stamped into every ckpt; check_ckpt_compat reads it
    }
    ...
    if args.resume and os.path.exists(args.resume):
        ...
        # v2 emb_offset FT: a legacy (absolute) ckpt has emb_pointer and no emb_offset. strict=False
        # transfers every SHARED weight (transformer / output heads / atom-r-hp-slot embeddings), DROPS
        # emb_pointer (unexpected), and leaves emb_offset fresh-initialised (missing). There is no
        # emb_pointer to resize here. (smoke check 4.)
        res = target.model.load_state_dict(ck["model"], strict=False)
        log.info(f"  FT load strict=False: missing(fresh-init)={list(res.missing_keys)}  "
                 f"unexpected(dropped)={list(res.unexpected_keys)}")
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        save_dict = {
            ...
            'arch_version': config.get('arch_version'),   # top-level for klrl_control.check_ckpt_compat
        }
